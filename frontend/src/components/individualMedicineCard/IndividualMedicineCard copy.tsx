...
class IndividualMedicineCard extends React.Component<IIndividualMedicineCardProps & RouteComponentProps<any>, IndividualMedicineCardState> {
    ...
    private pathAndCssStageMarker = {
        "執藥": "pack",
        "核藥": "verify",
        "派藥": "deliver"
    }

    // private recentDrugStage = (elderlyIndividualDrugRecordArray: IElderlyDrugStatus[]) => {
    //     const recent_drug_stage_status_id = elderlyIndividualDrugRecordArray.reduce((acc: Array<number>, data: IElderlyDrugStatus) => {
    //         return [...acc, data.status_id]
    //     }, [])
    //     const min_status_id = Math.min(...recent_drug_stage_status_id)
    //     const recent_stage = this.stage[min_status_id as 1 | 2 | 3]
    //     return recent_stage
    // }
    ...
    private handleFinishOnClick = () => {
        const allUsersId = this.props.elderlyDrugRecords.reduce((acc, data) => [...acc, data.elderly_id] as any, []) 
    ...
    public render = () => {
        ...
        return (
            <div className="individual-med-card-wrapper">
                                                ...
                                                {individual_drug_info.drug_details.map((drug_detail, index) => {
                                                    return (
                                                        <div key={index} data-status_id={drug_detail.drug_details_id}
                                                            ...
                                                            <span className="drug_time">{drug_detail.have_drug_time} {drug_detail.drug_details_id} </span>
